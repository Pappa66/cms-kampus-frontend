// cms-kampus-frontend/app/user/dosen/repository/page.tsx
'use client';
...
export default function DosenRepositoryManagementPage() {
  ...
  const [form, setForm] = useState<RepositoryForm>({
    ...
    abstract: '',
    prodi: '',
    isPublic: false, // Default tidak publik untuk Dosen
  });
  ...
  const fetchRepositories = async () => {
    ...
    try {
      ...
      // Perhatikan: localStorage.getItem('userId') mungkin belum terisi.
      // Anda harus pastikan ID pengguna disimpan di localStorage saat login
      // atau ambil dari token jika token mengandung ID pengguna.
      // Untuk sementara, kita akan anggap ada, atau Anda bisa tambahkan logic untuk mengurai token.
      const userId = localStorage.getItem('userId') || ''; // Asumsi userId disimpan
      const response = await fetchAPI(`/api/repositories?userId=${userId}`, 'GET', token);
      if (response.data) {
  ...
  if (currentUserRole !== 'DOSEN') {
    return (
      <UserLayout>
        <p className="text-red-500 text-center py-8">Anda tidak memiliki akses ke halaman ini. Hanya Dosen yang diizinkan.</p>
      </UserLayout>
    );
  }
  ...
  return (
    <UserLayout>
      <h1 className="text-2xl font-bold mb-6">Manajemen Repositori Dosen</h1>
